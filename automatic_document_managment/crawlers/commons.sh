...
function replace_var_in_var_last_month() {
    varValue=$1
    varValue="$(echo "${varValue}" | sed "s/{str{month_}}/$(LANG=fr_FR date --date='last month' +'%B')/g")"
    varValue="$(echo "${varValue}" | sed "s/{year_}/$(LANG=fr_FR date --date='last month' +'%Y')/g")"
    varValue="$(echo "${varValue}" | sed "s/{month_}/$(LANG=fr_FR date --date='last month' +'%m')/g")"
