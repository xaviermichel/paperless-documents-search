...
export class PdsSearchService {
  ...
  searchForPattern(
    ...
    return this.http.get<PdsSearchResultModel>(`${environment.sdsApiBaseUrl}/documents`, { params });
  }
  ...
  getSuggestionsForPattern(pattern: string): Observable<PdsSearchSuggestionsModel> {
    ...
    return this.http.get<PdsSearchSuggestionsModel>(`${environment.sdsApiBaseUrl}/documents/suggest`, {params});
  }

  getTopTermsRelatedToPattern(pattern: string): Observable<PdsAggregationsModel> {
    const params = new HttpParams().set('q', pattern);
    return this.http.get<PdsAggregationsModel>(`${environment.sdsApiBaseUrl}/documents/top_terms`, {params});
  }

  getAggregationsForPattern(pattern: string): Observable<PdsGlobalAggregationsWrapperModel> {
    const params = new HttpParams().set('q', pattern);
    return this.http.get<PdsGlobalAggregationsWrapperModel>(`${environment.sdsApiBaseUrl}/documents/aggregations`, {params});
  }
