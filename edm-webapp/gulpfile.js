...
gulp.task('backend-server', shell.task([
    'docker run --rm --name edm-backend-server-test --link paperless-documents-search-db-test -p 8053:8053 -e \'SPRING_APPLICATION_JSON={"spring.data.elasticsearch.cluster-nodes": "paperless-documents-search-db-test:9300" }\' -v $(pwd)/src/test/resources/documents:/host_mount_point -d paperless-documents-search-webapp'
]));
...
gulp.task('stop-backend-server', shell.task([
    'docker stop edm-backend-server-test'
]));
